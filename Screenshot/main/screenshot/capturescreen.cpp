...
{
	controlW = QSharedPointer<ControlWidget>(new ControlWidget(this));
	controlW->setWindowFlags(Qt::FramelessWindowHint);
	controlW->setAutoFillBackground(true);
	controlW->close();

	connect(controlW.data(), &ControlWidget::sigCancelPixmap, this, [=]() {
		exitPixmap();
	});

	connect(controlW.data(), &ControlWidget::sigFinishPixmap, this, [=]() {
		finishPixmap();
	});
...
{
	if (m_mousePressPoint == event->pos())
	{
		m_currentCaptureState = FinishMoveFullScreen;
		update();
		controlW->show();
		controlW->update();
	...
	else if (m_currentCaptureState == BeginMoveFullScreen && false == m_bMove)
	{
		m_currentCaptureState = FinishMoveFullScreen;
		update();
	}
...
void CaptureScreen::paintEvent(QPaintEvent *event)
{
	...
	case BeginMoveFullScreen:
	case FinishMoveFullScreen:
...
QRect CaptureScreen::getSelectRect()
{
	...
	else if (m_currentCaptureState == FinishMoveFullScreen)
	{
		return getRect(m_beginPoint, m_endPoint);
	}
...
void CaptureScreen::drawStretchRect()
{
	...
	controlW->move((bottomRight.x() - STRETCH_RECT_WIDTH / 2) - 85 , (bottomRight.y() - STRETCH_RECT_HEIGHT / 2) + 10);
...
QString CaptureScreen::getFileMd5(QString filePath)
{
...
QByteArray CaptureScreen::getBinary(QString filePath)
{
...
void CaptureScreen::keyPressEvent(QKeyEvent *event)
{
	...
	//	emit signalCompleteCature(m_capturePixmap);
		...
		QPixmap pix = m_capturePixmap;
		board->setImage(pix.toImage());
		close();
	}
}

void CaptureScreen::exitPixmap()
{
	controlW->close();
	this->close();
}
...
void CaptureScreen::savePixmap()
{
	...
	QString filename = QFileDialog::getSaveFileName(this, QStringLiteral("保存截图"), picName, "JPEG Files(*.png)");

	if (filename.length() > 0) {
		QImage pimage = m_capturePixmap.toImage();
		pimage.save(filename, "png");
	}
}

void CaptureScreen::finishPixmap()
{
	//生成图片名称
	QString picName;
	QTime time;
	...
	QString localPath = QString("./res/screen/%1").arg(picName);
	QImage pimage = m_capturePixmap.toImage();
	pimage.save(localPath, "png");
	QString ImageMd5 = getFileMd5(localPath);
	QByteArray ImageBinary = getBinary(localPath);

	//signalCompleteCature(m_capturePixmap, ImageMd5, ImageBinary);

	QClipboard *board = QApplication::clipboard();
	QPixmap pix = m_capturePixmap;
	board->setImage(pix.toImage());

	emit signalFinshPixmap(m_capturePixmap);
